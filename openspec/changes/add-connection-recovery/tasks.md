...
- [ ] 1.1 Add `src/RayTree.Core/Resilience/ConnectionRecoveryOptions.cs` — public record with `Enabled` (default `true`), `InitialDelay` (`1s`), `MaxDelay` (`30s`), `Factor` (`2.0`), `JitterFraction` (`0.2`), `MaxAttempts` (`null`); constructor-side validation throwing `ArgumentOutOfRangeException` for invalid values
- [ ] 1.2 Add `src/RayTree.Core/Resilience/ConnectionRetry.cs` — `internal static class` with single `RunAsync(Func<CancellationToken, Task> attempt, ConnectionRecoveryOptions options, string component, string endpoint, RayTreeMeter meter, ILogger logger, TimeProvider timeProvider, CancellationToken ct)` method; computes `min(InitialDelay × Factor^(attempt-1), MaxDelay)` with uniform `±JitterFraction` jitter per attempt; emits metric/log entries at documented levels
- [ ] 1.3 Add four new instruments to `src/RayTree.Core/Telemetry/RayTreeMeter.cs`: `raytree.connection.disconnects` (Counter<long>), `raytree.connection.recoveries` (Counter<long>), `raytree.connection.recovery.duration` (Histogram<double>, unit `"s"`), `raytree.connection.state` (ObservableGauge<int>); add internal helper methods `RecordConnectionDisconnect(component, endpoint)`, `RecordConnectionRecovery(component, endpoint, outcome, durationSeconds)`, `RegisterConnectionStateGauge(Func<IEnumerable<(string component, string endpoint, int state)>>)`
- [ ] 1.4 Add `InternalsVisibleTo` entries in `src/RayTree.Core/RayTree.Core.csproj` (if not already present) for `RayTree.Plugins.PostgreSQL` and `RayTree.Plugins.Kafka` so the plugins can call `ConnectionRetry.RunAsync` and the new meter helpers
...
- [ ] 2.2 `tests/RayTree.Core.Tests/Resilience/ConnectionRetryTests.cs` using `Microsoft.Extensions.Time.Testing.FakeTimeProvider` — asserts: first attempt runs immediately; backoff sequence `1s, 2s, 4s, 8s, 10s, 10s, …` when `MaxDelay = 10s` and `JitterFraction = 0`; jitter range is `[delay × 0.8, delay × 1.2]` for `JitterFraction = 0.2`; cancellation between attempts and during delay; `MaxAttempts` exhaustion rethrows last exception and records `outcome = "exhausted"` metric; success records `outcome = "succeeded"` metric; `Enabled = false` returns immediately without invoking the attempt delegate
...
- [ ] 3.3 Add `ReconnectAsync(CancellationToken ct)` method to `NotificationBasedPublisher` that wraps `ConnectionRetry.RunAsync` — attempt delegate disposes the broken connection, opens a fresh `NpgsqlConnection`, re-attaches the `Notification` event handler, issues `LISTEN {ChannelName}`, and swaps `_connection`
...
- [ ] 5.3 Wrap the existing lazy build path (`GetProducerAsync`) in `ConnectionRetry.RunAsync` when triggered after a fatal-error dispose, bounded by `ConnectionRecovery`; on success record `raytree.connection.recoveries{outcome="succeeded"}`
...
- [ ] 6.2 In `KafkaConsumer`'s poll thread loop, wrap `Consume` in `try`/`catch (KafkaException ex) when (ex.Error.IsFatal)`; on catch: dispose the current `IConsumer`, emit `raytree.connection.disconnects`, call `ConnectionRetry.RunAsync` (attempt delegate = build new `IConsumer` via existing init helper which re-runs the topic-wait probe; call `Subscribe`); on success update internal reference, emit `recoveries{outcome="succeeded"}`, resume polling
