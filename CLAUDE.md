...
- **`EntityChangeTracker`** — the single runtime host. Composes a `ChangePublisher` and a `ChangeSubscriber` internally. `InitializeAsync()` delegates to `ChangePublisher.InitializeAsync()` (which starts one `OutboxPublisherService` per entity type) then initializes registered consumer queues. Exposes `Consumers`, `ConsumeFromConsumerAsync`, and `ProcessMessageAsync` as the public subscriber API. `Register*` / `Get*` / `GetOutboxes` delegate to `ChangePublisher`. Exposes `Publisher` property for callers that need the `ChangePublisher` directly (e.g., `NotificationBasedPublisher`).
...
- **Tracker as thin coordinator**: `EntityChangeTracker` composes a `ChangePublisher` and a `ChangeSubscriber` — it owns neither set of registrations directly. `ChangeTrackingBuilder.BuildInternal()` creates the `ChangePublisher`, registers all plugins on it, constructs the tracker with it, then calls `ChangeSubscriberBuilder.Apply(tracker)` to attach the subscriber. `Register*`/`Get*`/`GetOutboxes` on the tracker are thin delegating methods. `ChangeSubscriber` and `ChangePublisher` both stay public for low-level testing but are not part of the primary API.
