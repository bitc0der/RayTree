...
- **`SubscriberOptions`** — `MaxRetries` (retry *attempts* after the first call), `RetryDelay`, `SkipOnFailure`, `DeduplicationRetention` (default 24 h — how old a processed `CorrelationId` must be before cleanup eligibility), `DeduplicationCleanupInterval` (default 1 h — how often `ChangeSubscriber` calls `IDeduplicationStore.CleanupAsync` after a successful message).
