...
public class InMemoryOutboxTests
{
    ...
    {
        var outbox = new InMemoryOutbox();
        var change1 = new EntityChange { EntityType = "EntityA", EntityId = "1", ChangeType = ChangeType.Insert };
        var change2 = new EntityChange { EntityType = "EntityB", EntityId = "2", ChangeType = ChangeType.Insert };

        await outbox.WriteAsync(change1);
        await outbox.WriteAsync(change2);

        var filtered = await outbox.GetUnpublishedAsync("EntityA");
    ...
    {
        var outbox = new InMemoryOutbox();
        var change1 = new EntityChange { EntityType = "TestEntity", EntityId = "1", ChangeType = ChangeType.Insert };
        var change2 = new EntityChange { EntityType = "TestEntity", EntityId = "2", ChangeType = ChangeType.Delete };

        await outbox.WriteAsync(change1);
        await outbox.WriteAsync(change2);

        var filtered = await outbox.GetUnpublishedAsync("TestEntity", changeType: ChangeType.Insert);
...
public class InMemoryQueueTests
{
    ...
    public async Task PublishAsync_MakesMessageAvailableForConsumption()
    {
        ...
        var change = new EntityChange { EntityType = "TestEntity", EntityId = "1", ChangeType = ChangeType.Insert };
        var payloadData = Encoding.UTF8.GetBytes("test payload");
